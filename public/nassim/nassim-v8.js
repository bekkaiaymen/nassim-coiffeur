// === Function now defined in HTML inline for better control ===
console.log('🚀 nassim.js v008 - suggestNearestAppointment defined in HTML');
// The function is defined in index.html <head> section for immediate availability
...
// Expose state for external scripts (like the inline suggest function)
window.nassimState = {
    get selectedServices() { return selectedServices; },
    get availableEmployees() { return availableEmployees; }
};
        ...
        <div class="service-card">
            ${hasValidImage
                ? `<div class="service-image" onclick="${service.hasVariants ? `openVariantsModal('${service._id}')` : `selectService('${service._id}')`}" style="width: 100px; height: 120px; border-radius: 15px; overflow: hidden; margin-left: 15px; flex-shrink: 0; background: linear-gradient(135deg, #1a1a1a 0%, #2d2d2d 100%); position: relative; box-shadow: 0 4px 15px rgba(203, 163, 92, 0.3); border: 2px solid rgba(203, 163, 92, 0.2); cursor: pointer;">
                    <img src="${service.image}" alt="${service.name}" style="width: 100%; height: 100%; object-fit: cover;" onerror="console.log('Failed to load service image:', this.src); this.parentElement.outerHTML='<div class=\\'service-icon\\' onclick=\\'${service.hasVariants ? `openVariantsModal(\\'${service._id}\\')` : `selectService(\\'${service._id}\\')`}\\' style=\\'cursor: pointer;\\'>${getServiceIcon(service.name)}</div>';">
...
function displayEmployees(employees) {
    ...
    if (!employees || employees.length === 0) {
        container.innerHTML = '<div class="empty-state">لا يوجد حلاقين</div>';
        return;
    }
...
const TIMELINE_PIXELS_PER_MINUTE = 6;

async function loadTimelineData(date) {
    try {
        // Get appointments
        const apptResponse = await fetch(`/api/appointments/public?date=${date}`);
        ...
        const empResponse = await fetch(`/api/employees/available`);
        let availableEmployees = [];
        if (empResponse.ok) {
        ...
        renderTimelineStrip(date, appointments, availableEmployees);
...
function renderTimelineStrip(date, appointments, availableEmployees = []) {
